...
void LoginDialog::initTelcomWidget()
{
    ...
    icon_label->setPixmap(QPixmap(":/img/telecom"));
...
void LoginDialog::translateLanguage()
{
    ...
    safe_label->setText(tr("oneclick"));
    protect_label->setText(tr("terminal service"));
	...
	case SAFE:
		{
			...
			this->changeTitle();
			this->turnWidget();
            renren_button->setIcon(QIcon(":/img/telecom"));
		}
		break;
	case RENREN:
		{
	...
	case SAFE:
		{
			...
			this->turnWidget();
			sina_login = SAFE;
            sina_button->setIcon(QIcon(":/img/telecom"));
		}
		break;
	case RENREN:
		{
